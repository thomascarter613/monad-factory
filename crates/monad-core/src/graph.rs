...
impl RepositoryGraph {
    ...
    pub fn is_complete(&self) -> bool {
        self.nodes
            .iter()
            .all(|node| node.status == CheckStatus::Pass)
    }
...
mod tests {
    ...
    fn parses_supported_formats() {
        assert_eq!(
            GraphRenderFormat::parse("text"),
            Some(GraphRenderFormat::Text)
        );
        assert_eq!(
            GraphRenderFormat::parse("json"),
            Some(GraphRenderFormat::Json)
        );
        assert_eq!(
        ...
        assert_eq!(
            GraphRenderFormat::parse("dot"),
            Some(GraphRenderFormat::Dot)
        );
    ...
    fn renders_static_graph_without_external_dependencies() {
        ...
        assert!(graph
            .render(GraphRenderFormat::Text)
            .contains("engine: native"));
        assert!(graph
            .render(GraphRenderFormat::Json)
            .contains("\"engine\": \"native\""));
        assert!(graph
            .render(GraphRenderFormat::Mermaid)
            .contains("graph TD"));
        assert!(graph
            .render(GraphRenderFormat::Dot)
            .contains("digraph monad_factory"));
    }
}
